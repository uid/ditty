var jsonPatterns = {
  ...
  "alert" : {
    ...
    meaning: {
      native_invocation: {
        ...
        args: { "message" : { reference: { name: "message" } } }
      }
    }
  },
  "play" : {
  ...
  //       args: { "question" : { reference: { name: "question" } } }
  //     }
  //   }
  // },
  "loop" : {
    ...
    args: [{ name: "actions", type: "instructions" }],
    meaning: {
      native_invocation: {
        ...
        args: { "actions" : { reference: { name: "actions" } } }
      }
    }
  },
  "loop-breaker" : {
    ...
    meaning: {
      ...
      // native_invocation: {
      //   implementation: '(function(c, e, os, args) { c(new ExceptionMeaning("loop-breaker")) })',
      //   args: {}
      // }
    }
  },
  ...
  //   ],
  //   args: [{ name: "exception" }, { name: "action" }],
  //   meaning: {
  //     native_invocation: {
  ...
  "if" : {
    ...
    args: [{ name: "condition" }, { name: "actions", type: "instructions" }],
    meaning: {
      native_invocation: {
  ...
  //   ],
  //   args: [{ name: "value 1" }, { name: "value 2" }],
  //   meaning: {
  //     native_invocation: {
  ...
  //       args: { "value" : { reference: { name: "value" } } }
  //     }
  //   }
  // },
  "is-false" : {
  ...
  "note-do" : { id: "note-do", representations: [ { template: "Do" } ], args: [], meaning: { native_invocation: { implementation: '(function(c, e, os, args) { c(new NumberMeaning(mtof(65))) })', args: {} } } },
  "note-re" : { id: "note-re", representations: [ { template: "Re" } ], args: [], meaning: { native_invocation: { implementation: '(function(c, e, os, args) { c(new NumberMeaning(mtof(67))) })', args: {} } } },
  "note-mi" : { id: "note-mi", representations: [ { template: "Mi" } ], args: [], meaning: { native_invocation: { implementation: '(function(c, e, os, args) { c(new NumberMeaning(mtof(69))) })', args: {} } } },
  "true" : {
    id: "true",
    representations: [
      { template: "true" }
  ...
  "save-var" : {
    ...
    args: [{ name: "action" }, { name: "variable name" }],
    meaning: {
      native_invocation: {
  ...
  //     { template: "[text 1][text 2][text 3]" }
  //   ],
  //   args: [{ name: "text 1" }, { name: "text 2" }, { name: "text 3" }],
  //   meaning: {
  //     invocation: {
  //       pattern: "concat",
  //       args: {
          ...
          "actions" : [
            {
              invocation: {
                pattern: "if",
                args: {
                  condition: { invocation: { pattern: "is-false", args: { "value" : { reference: { name: "condition" } } } } },
  ...
  "maybe-block" : {
    ...
    meaning: {
      invocation: {
        pattern: "if",
        args: {
          condition: { invocation: { pattern: "maybe", args: {} } },
          actions: { reference: { name: "actions" } }
        }
      }
    }
  ...
  //   ],
  //   args: [{ name: "stop condition" }, { name: "actions", type: "instructions" }],
  //   meaning: {
  //     invocation: {
  //       pattern: "loop",
  //       args: {
  ...
  // "user-says-stop" : {
  //   id: "user-says-stop",
  //   representations: [{ template: "the user says to stop" }],
  //   args: [],
  //   meaning: {
  //     native_invocation: {
  //       implementation: '(function(os, args) { })',
  //       args: {}
  //     }
  //   }
  // },
  // "play" : {
  //   id: "play",
  //   representations: [{ template: "play [instrument]" }],
  //   args: [{ name: "instrument" }],
  //   meaning: {
  //     native_invocation: {
  //       implementation: '(function(os, args) { })',
  //       args: {}
  ...
  // "hi-hat" : {
  //   id: "hi-hat",
  //   representations: [{ template: "hi-hat" }, { template: "the hi-hat" }],
  //   args: [],
  //   meaning: {
  //     native_invocation: {
  //       implementation: '(function(os, args) { })',
  ...
  // "every" : {
  //   id: "every",
  //   representations: [{ template: "[actions] every [event]" }],
  //   args: [{ name: "actions", type: "instructions" }, { name: "event" }],
  //   meaning: {
  //     native_invocation: {
  //       implementation: '(function(os, args) { })',
  //       args: {}
  ...
  // "quarter-note" : {
  //   id: "quarter-note",
  //   representations: [{ template: "quarter note" }],
  //   args: [],
  //   meaning: {
  //     native_invocation: {
  //       implementation: '(function(os, args) { })',
  ...
  // "for-each" : {
  //   id: "for-each",
  //   representations: [{ template: "For each [item] in [collection], [actions]." }],
  //   args: [{ name: "actions", type: "instructions" }, { name: "item" }, { name: "collection" }],
  //   meaning: {
  //     native_invocation: {
  //       implementation: '(function(os, args) { })',
  //       args: {}
  ...
  // "for" : {
  //   id: "for",
  //   representations: [{ template: "for([initializer]; [condition]; [post-action]) { [actions] }" }],
  //   args: [{ name: "actions", type: "instructions" }, { name: "initializer" }, { name: "condition" }, { name: "post-action" }],
  //   meaning: {
  //     native_invocation: {
  //       implementation: '(function(os, args) { })',
  //       args: {}
...
function compilePatterns(json) {
  for(var i in json) {
    try {
      patterns[i] = new JsonPatternUnarchiver().unarchive(jsonPatterns[i])
    } catch(e) {
...
JsonPatternUnarchiver.prototype.unarchive = function(json) {
  ...
  var args = _.inject(_.map(json["args"], this._arg.bind(this)), function(obj, ref) { obj[ref.name] = ref; return obj }, {})
  var meaning = this._meaning(json["meaning"])
  return new Pattern(this.id, representations, args, meaning)
}
...
JsonPatternUnarchiver.prototype._nativeInvocation = function(json) {
  ...
  var args = {}
  for(var argName in json["args"]) {
    args[argName] = this._meaning(json["args"][argName])
  }
...
_BasicPattern.prototype._checkRepresentation = function(repr) {
  ...
  if(!setEq(argNames, repr.args)) {
    throw new Error("representation has mismatched arguments")
  }
}
...
function Pattern(id, representations, references, meaning) {
  this.id = id
...
Pattern.prototype.apply = function(args) {
  ...
  for(var i in args) {
    if(!(i in this.references)) {
      throw new Error("argument '" + i + "' doesn't exist in pattern '" + this.id + "'")
    }
...
var clicker = new ModalStrike(audioContext)
var bar = new ModalBar(audioContext)
...
$(function() {
  ...
  compilePatterns(jsonPatterns) // decodes patterns from JSON
